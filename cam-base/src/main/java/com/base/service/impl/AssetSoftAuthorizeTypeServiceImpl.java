...
import com.base.dao.AssetSoftAuthorizeTypeDAO;
import com.base.service.DataBaseService;

public class AssetSoftAuthorizeTypeServiceImpl extends BaseService implements DataBaseService {
	...
	public int save(Map<String, Object> map) {
		...
		if (checkMap(map)) {
			...
			try {
				...
				AssetSoftAuthorizeTypeDAO mapper = sqlSession.getMapper(AssetSoftAuthorizeTypeDAO.class);
				result = mapper.save(map);
				if (result > 0 && map.get("authorizeTypeId") != null) {
					result = Integer.parseInt(map.get("authorizeTypeId").toString());
				}
			} catch (IOException e) {
	...
	private boolean checkMap(Map<String, Object> map) {
		if (map.get("authorizeName") == null) {
			return false;
		}
		if (map.get("onlyOne") == null) {
			return false;
		}
		if (map.get("needCount") == null) {
			return false;
		}
		return true;
	}
